...
foreach ($result as $row) {
    ...
    if ($row["attendance_status"] == "MLeave") {
        $total_mleave++;
        $status = '<span class="badge badge-info">Medical Leave</span>';
    }
    if ($row["attendance_status"] == "CLeave") {
        $total_cleave++;
        $status = '<span class="badge badge-warning">Casual Leave</span>';
    }
    ...
    $mleave_percentage = ($total_mleave / $total_row) * 100;
    $cleave_percentage = ($total_cleave / $total_row) * 100;
...
<script type="text/javascript">
    ...
    function drawChart() {
        var data = google.visualization.arrayToDataTable([
            ...
            ['MLeave', <?php echo $mleave_percentage; ?>],
            ['CLeave', <?php echo $cleave_percentage; ?>]
        ]);
